...
/**
 * Renders assistant and user text as readable structure.
 *
  ...
  while (index < lines.length) {
    ...
    if (isTableRow(trimmed)) {
      ...
      while (index < lines.length && isTableRow(lines[index].trim())) {
        if (!isDivider(lines[index].trim())) rows.push(cells(lines[index].trim()));
        index += 1;
      }
        ...
        if (block.kind === "quote")
          return (
            <p className="note" key={key}>
              <span>{inline(block.lines.join(" "), key)}</span>
            </p>
          );
        if (block.kind === "table") {
