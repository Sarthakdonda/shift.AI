...
class FakeGemini:
    ...
    def generate_structured(self, instruction, context, schema):
        ...
        source_text = context.get('latest_user_message', {}).get('content', 'Synthetic test evidence') if isinstance(context, dict) else 'Synthetic test evidence'
        readiness = [{'criterion': criterion, 'source_id': source_id, 'quote': source_text} for criterion in ('problem', 'workflow', 'outcome')]
        values = {
            'Discovery': {'collected_information': [{'category': 'workflow', 'topic': 'workflow.entry', 'fact': 'Staff retype invoice data', 'source': 'User message', 'source_ids': [source_id]}], 'missing_information': [], 'critical_missing': [], 'scores': {k: 80 for k in Scores.model_fields}, 'enough_information': True, 'next_question': '', 'next_questions': [], 'assumptions': [], 'unknowns': [], 'answered_topics': [], 'information_sufficiency': 80, 'readiness_reason': 'Test fixture supplies the workflow, problem and outcome.', 'readiness_evidence': readiness},
